...
- 新增"模板市场"页面（侧边栏"模板市场"）：
  - **当前模板**：管理本地配置模板（新建/编辑/删除，支持从官方模板导出起点），一键切换订阅生成所用的模板
  - **模板市场**：对接自部署的市场服务（独立私有仓库 `sing-box-template-server`），支持搜索/排序/详情预览/下载使用
  - **我的发布**：模板以匿名方式发布到市场，管理员审核通过后上架；可更新发布、下架删除、跟踪审核状态（拒绝理由可见）
- 订阅下载/手动添加/自动更新统一使用"当前模板"生成配置：默认官方内置骨架；切换为在线模板后新配置由模板内容 + 订阅节点生成（模板内 `{{NODES}}` 占位符标记节点注入位置，未标注时自动追加注入）
- 在线模板缺失 gRPC API 服务 / mixed 入站 / 日志输出时自动补齐，端口始终与应用设置对齐；模板配置在设置变更时仅同步端口，不被官方高级选项覆盖
- 模板市场服务端独立部署（PostgreSQL，匿名发布 + 管理员审核，Docker Compose 一键部署），见 sing-box-template-server 仓库 README
- 模板编辑器新增"可视化配置"模式：基于官方骨架开放关键配置——默认出站、业务分流组（Telegram/YouTube/Netflix/OpenAI/Google 按组开关）、广告拦截/DNS 劫持/mDNS/Fake DNS、三层 DNS 地址、自定义分流规则（域名/域名后缀/关键字/IP 段 → 走代理/直连/拦截，优先级高于内置国内直连），无需手写 JSON；骨架不支持表单表达的模板自动回退 JSON 模式
- 模板市场服务地址内置默认值（发布构建指向线上服务，开发构建自动使用本地 127.0.0.1:8787 调试地址），清空并保存即可恢复默认
