...
### ✨ 新功能
- **更新通道能力增强** - 新增 `stable / prerelease / autobuild` 三种更新通道，前后端联动支持按通道检查更新
- **备份与恢复** - 新增应用状态快照导出/导入能力，支持 `dry-run` 预检并在恢复后自动同步运行态
- **订阅健康可观测性** - 自动更新增加失败计数、错误分类、退避截止时间等健康字段，并在订阅页展示失败与重试信息

### 🔧 优化改进
- **事件契约强类型化** - 内核生命周期/操作事件 payload 收敛到明确类型，`subscription-updated` 载荷类型同步收敛
- **状态模型与展示收敛** - 清理 AppStore 遗留 WebSocket 检测状态，避免重复状态源带来的维护成本
- **IPC 参数规范化** - 清理部分命令参数 `camelCase/snake_case` 双写，统一前端调用约定
- **设置页能力补齐** - 设置页新增“备份与恢复”模块，支持导出、预检恢复与执行恢复
- **发布流程重构** - 发布流水线拆分为“Tag 触发预发布构建”和“手动转正式”两阶段，且两阶段均自动读取 `docs/CHANGELOG.md` 对应版本作为 Release Notes

### 🐛 问题修复
- **插件重复注册修复** - 修复后端入口 `tauri_plugin_websocket` 重复注册问题
- **启动隐藏分支稳健性修复** - `--hide` 启动路径移除 `unwrap`，改为安全告警日志，避免异常场景崩溃
- **自动更新覆盖修复** - 修复多订阅自动刷新时误覆盖 `active_config_path` 的风险，仅对当前生效订阅应用运行态变更
- **备份兼容回退修复** - 恢复旧备份缺少 `update_channel` 字段时自动回退 `stable`，保证配置可用

### 🔒 安全加固
- **权限收敛** - 默认 capability 从 `core:default` 收敛为显式所需权限，降低权限面
- **CSP 加固** - `tauri.conf.json` 明确设置 CSP，不再使用 `null`
