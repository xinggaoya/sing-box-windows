...
impl<R: Send + Sync + 'static> WebSocketRelay<R> {
    pub fn new<F>(
        window: Window,
        endpoint: &str,
        event_name: &str,
    ...
    pub async fn start(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
                    ...
                    Ok(Message::Text(text)) => {
                        match serde_json::from_str::<Value>(&text) {
                            Ok(data) => {
                                ...
                                // 发送到前端
                                if let Err(e) = window.emit(&event_name, &parsed_data) {
        ...
        tokio::select! {
            _ = receive_task => {
                info!("{}接收任务结束", event_name);
            }
            _ = send_task => {
                info!("{}发送任务结束", event_name);
            }
        }
...
/// 启动通用中继器的便捷函数
pub async fn start_relay_with_retry<R: Send + Sync + 'static>(
    relay: WebSocketRelay<R>,
    relay_type: &str,
    ...
    loop {
        match relay.start().await {
            Ok(_) => {
                info!("{}中继器正常结束", relay_type);
                break Ok(());
            }
            Err(e) => {
                retry_count += 1;
                if retry_count >= max_retries {
                    error!("{}中继器重试{}次后仍然失败: {}", relay_type, max_retries, e);
                    break Err(e);
                }
                
                warn!("{}中继器失败，{}秒后重试 ({}/{}): {}", relay_type, retry_delay.as_secs(), retry_count, max_retries, e);
