...
├── template_marketplace/  # 在线模板：本地 KV 存储、市场客户端（独立私有服务端）、17 个命令
...
| 模板/市场         | `template_marketplace/commands.rs` + `local_store.rs` | 17 个命令，存 `generic_config` 表 key=`config_templates`/`template_market_settings` |
