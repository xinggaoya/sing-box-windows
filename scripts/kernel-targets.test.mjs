...
test('getKernelResourceMap 支持 Windows ARM64 目标', () => {
  const root = path.join('tmp', 'kernel-root')
  const map = getKernelResourceMap('windows', 'arm64', root)

  const keys = Object.keys(map)
  assert.equal(keys.length, 2)

  const executable = path.resolve(root, 'windows', 'arm64', 'sing-box.exe')
  const versionFile = path.resolve(root, 'windows', 'arm64', 'version.txt')

  assert.equal(map[executable], 'kernel/windows/arm64/sing-box.exe')
  assert.equal(map[versionFile], 'kernel/windows/arm64/version.txt')
})
