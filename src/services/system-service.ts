...
export const systemService = {
    ...
    backupExportSnapshot(filePath?: string) {
        const payload = filePath && filePath.trim().length > 0 ? { filePath: filePath.trim() } : undefined
        return invokeWithAppContext<BackupExportResult>('backup_export_snapshot', payload, { skipDataRestore: true })
    },
