export default {
  ...
  sub: {
    ...
    originalConfigWarning: 'При использовании исходной конфигурации расширенные параметры (DNS/маршрутизация/реклама и т.д.) не применяются',
    configPathCollision: 'Обнаружен дубликат пути конфигурации. Конфигурация будет пересоздана перед переключением.',
    urlSubscription: 'URL подписки',
    manualConfig: 'JSON конфигурация (оригинал)',
    uriList: 'Ссылки / YAML',
    uriContent: 'Содержимое узлов',
    uriContentPlaceholder: 'Поддерживаются vmess://, vless://, trojan://, ss://, hysteria2:// (по одному в строке) или Clash/Mihomo YAML',
    ...
    manualHint: 'Только sing-box JSON. По умолчанию используется исходная конфигурация с заменой портов',
    uriHint: 'Поддерживаются vmess/vless/trojan/ss/hysteria2 (по одному в строке) или Clash/Mihomo YAML',
    ...
    autoUpdateHealthBackoff: 'Автообновление не удалось {count} раз, повтор через {remaining}: {reason}',
  ...
  setting: {
    ...
    backup: {
      ...
      description: 'Экспорт текущего состояния приложения или восстановление настроек и подписок из резервной копии',
      restoreHint: 'Восстановление перезапишет текущие настройки, канал обновлений и список подписок',
    ...
    kernel: {
      ...
      embeddedHint: 'Рекомендуемая версия ядра уже встроена в приложение. Ручное обновление используйте только для диагностики или тестов.',
      version: 'Версия ядра',
      ...
      manualImportDesc: 'Перетащите файл в окно или выберите локальный файл. Поддерживаются исполняемые файлы (Windows exe / macOS и Linux бинарные) и архивы (zip/tar.gz/tgz/tar).',
    ...
    singboxProfile: {
      ...
      originalConfigHint: 'Текущая подписка использует исходную конфигурацию, поэтому расширенные настройки ниже не будут записаны',
